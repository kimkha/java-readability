...
public class Readability {
    ...
    private boolean detectNextLink;
    ...
    public void processDocument(String url) throws PageReadException {
        ...
        String content = pageReader.readPage(givenUrl);

        document = Jsoup.parse(content, givenUrl);

        if (document.getElementsByTag("body").size() == 0) {
            Log.e(TAG, "no body to parse " + givenUrl);
            impossible = true;
    ...
    private void prepDocument() {
        ...
        if (frames.size() > 0) {
            Log.e(TAG, "Frames. Can't deal. Write code later to look at URLs and fetch");
            impossible = true;
            return;
        }
    ...
    private void startParsing() {
        ...
        if (readAllPages || detectNextLink) {
    ...
    private Element grabArticle(Element pageElement) {
        ...
        while (elIterator.hasNext()) {
            ...
            if (stripUnlikelyCandidates) {
                ...
                if (Patterns.exists(Patterns.UNLIKELY_CANDIDATES, unlikelyMatchString)
                    ...
                    Log.d(TAG, "Removing unlikely candidate - " + unlikelyMatchString);
        ...
        for (Element candidate : candidates) {
            ...
            Log.d(TAG, "Candidate [" + candidate.getClass() + "] (" + candidate.className() + ":"
        ...
        for (Element siblingNode : siblingNodes) {
            ...
            Log.d(TAG, "Looking at sibling node: [" + siblingNode.getClass() + "] (" + siblingNode.className()
                    + ":" + siblingNode.id() + ")");
            if (scored) {
                Log.d(TAG, "Sibling has score " + getContentScore(siblingNode));
            } else {
                Log.d(TAG, "Sibling has score unknown");
            }
            ...
            if (append) {
                ...
                if (!"div".equals(siblingNode.tagName()) && !"p".equals(siblingNode.tagName())) {
                    ...
                    Log.d(TAG, "Altering siblingNode of " + siblingNode.tagName() + " to div.");
    ...
    private void cleanConditionally(Element e, String tag) {
        ...
        for (int i = curTagsLength - 1; i >= 0; i--) {
            ...
            Log.d(TAG, "Cleaning Conditionally [" + ee.getClass() + "] (" + ee.className() + ":" + ee.id()
                    + ")" + contentScore);

            if (weight + contentScore < 0) {
                Log.d(TAG, "Negative content score");
                ee.remove();
    ...
    private URI findBaseUrl0(String stringUrl) throws URISyntaxException {
        ...
        int pathLength = cleanedPath.endsWith("/") ? cleanedPath.length() - 1 : cleanedPath.length();
        return new URI(url.getScheme(), url.getAuthority(),
                "/" + cleanedPath.substring(0, pathLength), null, null);
    }
    ...
    private String findNextPageLink(Element body) {
        ...
        for (Element link : allLinks) {
            ...
            String linkData = linkText + " " + link.className() + " " + link.id()
                    + " " + link.attr("rel"); // new from Dec 13, 2015: Some page use 'rel'
            ...
            while (parentNode != null) {
                String parentNodeClassAndId = parentNode.className() + " " + parentNode.id()
                        + " " + parentNode.attr("rel"); // new from Dec 13, 2015: Some page use 'rel'
                if (!positiveNodeMatch && Patterns.exists(Patterns.PAGINATION, parentNodeClassAndId)) {
                    positiveNodeMatch = true;
                    linkObj.incrementScore(25);
                }
                if (!negativeNodeMatch && Patterns.exists(Patterns.NEGATIVE, parentNodeClassAndId)) {
                    /*
    ...
    public boolean isDetectNextLink() {
        return detectNextLink;
    }

    public void setDetectNextLink(boolean detectNextLink) {
        this.detectNextLink = detectNextLink;
    }
}
